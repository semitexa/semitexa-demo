...
final class ApiErrorTriggerHandler implements TypedHandlerInterface
{
    ...
    public function handle(ApiErrorTriggerPayload $payload, DemoFeatureResource $resource): DemoFeatureResource
    {
        ...
        // JSON clients receive the structured error envelope through the exception pipeline.
        ...
        $spec = new FeatureSpec(
            ...
            entryLine: 'API failures should stay operationally useful. This page shows the exact error body a client would parse, not just the fact that the request failed.',
            learnMoreLabel: 'Inspect error envelope →',
            deepDiveLabel: 'Error mapping internals →',
            relatedSlugs: [],
    ...
    /**
     * @return array{0: string, 1: string, 2: list<array{concern: string, behavior: string, why: string}>}
     */
    private function envelopeCopy(string $type): array
    {
        return match ($type) {
            'validation' => [
                'Validation envelope',
                'Field-level contract failures stay machine-readable instead of collapsing into one opaque message.',
                [['concern' => 'Field errors', 'behavior' => 'Validation problems stay nested under `error.context.fields`.', 'why' => 'Clients can bind UI or retry logic to structured field feedback.']],
            ],
            'forbidden' => [
                'Forbidden envelope',
                'Authorization failures keep the same outer shape, so clients do not need a second parsing path.',
                [['concern' => 'Auth semantics', 'behavior' => 'The HTTP status changes, but the JSON envelope stays identical in shape.', 'why' => 'SDKs and dashboards can branch on `error.code` with one parser.']],
            ],
            'conflict' => [
                'Conflict envelope',
                'State conflicts travel through the same error contract without leaking framework internals.',
                [['concern' => 'State mismatch', 'behavior' => 'Conflict errors remain explicit and parsable at the contract level.', 'why' => 'Client retry or fallback decisions should not require message scraping.']],
            ],
            'rate-limit' => [
                'Rate-limit envelope',
                'Retry semantics stay visible in both the header and the machine-readable body context.',
                [['concern' => 'Retry guidance', 'behavior' => '`Retry-After` and `error.context.retry_after` both carry the wait time.', 'why' => 'Infrastructure and application clients can consume the same signal at different layers.']],
            ],
            default => [
                'Not-found envelope',
                'The route throws a domain exception, but the external API pipeline still returns one predictable error envelope.',
                [
                    ['concern' => 'Exception mapping', 'behavior' => 'A typed domain exception becomes `{ error: { code, message, context, request_id } }`.', 'why' => 'Clients can rely on error structure even when the business path fails.'],
                    ['concern' => 'Context preservation', 'behavior' => 'Resource name and identifier stay in `error.context` instead of being buried in text.', 'why' => 'Support tooling can diagnose failures without brittle string parsing.'],
                ],
            ],
        };
    }
