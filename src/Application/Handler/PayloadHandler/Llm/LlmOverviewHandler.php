    ...
    public function handle(LlmOverviewPayload $payload, DemoFeatureResource $resource): DemoFeatureResource
    {
        ...
        return $this->projector->project($resource, $spec)
            ->withResultPreviewTemplate('@project-layouts-semitexa-demo/components/previews/cli-command-workbench.html.twig', [
                ...
                'pillars' => [
                    ['title' => 'Your commands stay primary.', 'summary' => 'A skill starts as an ordinary Semitexa console command owned by your project or package.'],
                    ['title' => 'Metadata makes it usable.', 'summary' => '`#[AsAiSkill]` adds summary, use/avoid guidance, confirmation mode, and argument policy.'],
                    ['title' => 'The manifest stays reviewable.', 'summary' => 'Only the commands you expose become part of the skill manifest the assistant may use.'],
                ],
                'commands' => [
                    ['name' => '#[AsCommand(...)]', 'purpose' => 'Define the real console command first.', 'value' => 'The assistant extends your CLI; it does not replace it with a second hidden automation layer.'],
                    ['name' => '#[AsAiSkill(...)]', 'purpose' => 'Attach AI metadata to the command you want to expose.', 'value' => 'This is where you define risk, confirmation, exposed arguments, and usage guidance.'],
                    ['name' => 'bin/semitexa ai:skills --json', 'purpose' => 'Verify what the assistant can actually see.', 'value' => 'Treat the manifest as the review surface for your project-specific skills.'],
                ],
                'snippets' => [
                    ['label' => 'Minimal custom skill', 'code' => "#[AsCommand(name: 'reports:rebuild')]\n#[AsAiSkill(summary: 'Rebuild reporting snapshots', argumentPolicy: 'allowlisted')]\nfinal class RebuildReportsCommand extends Command {}"],
                    ['label' => 'Review the exposed manifest', 'code' => "bin/semitexa ai:skills --json"],
                ],
            ])
